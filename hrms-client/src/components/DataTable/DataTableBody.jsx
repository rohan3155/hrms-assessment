const DataTableBody = ({ columns, rows, actions, isLoading }) => {
	...
	if (!rows || rows.length === 0) {
		return (
			<tbody>
				<tr>
					<td colSpan={columns.length + (actions ? 1 : 0)} className="px-6 py-8 text-center text-zinc-500 dark:text-zinc-400">
						No data found.
					</td>
				</tr>
			</tbody>
	...
	return (
		<tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
			{rows.map((row, i) => (
				<tr key={i} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
					{columns.map(col => (
						<td key={col.key} className="px-6 py-4 whitespace-nowrap text-sm text-zinc-700 dark:text-zinc-300">
